 ...
 * @@Modify Date: 2017-08-15 21:25:37
 * @@Function:
 */
...
if (is_admin()) {
    // return;
    if(!class_exists('Magiccart')) return;
    if (class_exists('ReduxFrameworkPlugin')) new Magiccart\Core\Block\Themecfg;
} else {
    if(!class_exists('Magiccart')){
        echo '<div class="message error woocommerce-error"><p>This theme by <a href="http://alothemes.com/" target="_blank">Alothemes</a>. It requires Magiccart plugin in order to work.</p></div>';
    } else {

        // function hello_world() {
        //     echo "Hello World!";
        // }
        // add_shortcode( 'Magiccart\Cms\Block\Block', 'hello_world' );
        // do_shortcode('[Magiccart\Cms\Block\Block]');

        // $block = new Magiccart\Cms\Block\Block;
        // $block = new Magiccart\Core\Block\Shortcode;        
    }

    // require 'inc/magiccart-functions.php';
    require 'inc/magiccart-template-hooks.php';
    require 'inc/magiccart-template-functions.php';
    require_once('inc/post-like.php');
}

if(class_exists('Magiccart\Megamenu\Controller\Megamenu')){
     $megamenu = new Magiccart\Megamenu\Controller\Megamenu;
}
...
{
    global $alothemes;
    return $alothemes->get_options($key);
}
...
function breadcrumbs(){ 
    echo '<ul class="breadcrumbs" >';
        echo '<li class="trail-item trail-begin"><a href="' . home_url() .'">'; 
            esc_html_e('Home','alothemes');
        echo '</a></li>';
        if( is_tag() ) { 
            esc_html_e('Posts Tagged &quot;','alothemes');
            single_tag_title();
        } elseif (is_day()){
            esc_html_e('Posts made in','alothemes');
            the_time('F jS, Y');
        } elseif (is_month()) {
            esc_html_e('Posts made in','alothemes');
            the_time('F, Y');
            ...
            esc_html_e('Search results for','alothemes');
            the_search_query();
        } elseif (is_single()) {
            ...
            esc_html_e('Posts by ','alothemes'); 
            echo ' ' . $curauth->nickname;

        } elseif (is_page()) {
            echo "<li class='trail-item'>" .  wp_title('', false) . "</li>";
        }
...
function getGridStyle($selector = ' .woocommerce ul.products.grid li.product')
{
    ...
    $responsive = array();
    if (is_shop()) {  //if (function_exists('is_shop') && is_shop()) {
    ...
    if (isset($_GET['visible']))
        $responsive['visible'] = $_GET['visible']; // USED DEMO
