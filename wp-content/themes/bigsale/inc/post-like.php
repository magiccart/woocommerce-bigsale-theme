...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( 'Like', 'alothemes' ),
		'unlike' => __( 'Unlike', 'alothemes' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'alothemes' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'alothemes' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'alothemes' );
		$icon = $icon_empty;
	}
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'alothemes' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'alothemes' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
